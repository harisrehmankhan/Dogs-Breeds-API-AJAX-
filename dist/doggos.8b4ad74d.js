...
var DOG_URL = "https://dog.ceo/api/breeds/image/random"; // status, message;
...
document.querySelector(".add-doggo").addEventListener("click", addNewDogo); // Start Breed Select Lis API

var BREED_URL = "https://dog.ceo/api/breeds/list/all";
var dogfr = document.querySelector(".dog-frame");
var prom = fetch(BREED_URL);
prom.then(function (res) {
  var proc = res.json();
  return proc;
}).then(function (prprom) {
  var ur = prprom.message;
  var sel = document.createElement("select");
  sel.id = "salact";
  sel.style = "margin-left: 612px; margin-bottom: 10px;";
  dogfr.appendChild(sel); //    console.log(Object.keys(ur)[1]);

  for (i = 0; i < Object.keys(ur).length; i++) {
    var op = document.createElement("option");
    spval[i] = Object.keys(ur)[i];
    op.value = Object.keys(ur)[i];
    op.innerHTML = Object.keys(ur)[i]; // console.log(Object.keys(ur)[i]);
